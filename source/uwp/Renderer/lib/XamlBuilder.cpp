...
namespace AdaptiveCards { namespace Rendering { namespace Uwp
{
        ...
        ITextBlock * textBlock)
    {
        ...
        HRESULT hr = xmlDocumentIO->LoadXml(textIn);
        if (FAILED(hr))
        {
        ...
        else
        {
            ...
            while (childNode != nullptr)
            {
                ...
                {
                    nodeHandled = SUCCEEDED(AddListInlines(childNode.Get(), (isOrderedListResult == 0), inlines.Get()));
                }
                else if (isLinkResult == 0)
                {
                    nodeHandled = SUCCEEDED(AddLinkInline(childNode.Get(), inlines.Get()));
                }
